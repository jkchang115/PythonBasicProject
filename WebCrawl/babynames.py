...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    # add the data if name not in the name_data dict
    if name not in name_data:
        name_data[name] = {year: rank}
    else:
        # add if year not in name_data dict
        if year not in name_data[name]:
            name_data[name][year] = rank
        # replace the rank with smaller one if the same year
...
def search_names(name_data, target):
    ...
    for name in name_data:
        if target_upper in name.upper():
            ...
            final_list.append(formatted_name)
    return final_list
